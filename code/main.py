...
def check_lib(lib, unique_words, word_to_books, sort_output=True):
    dw1 = lib.distinct_words("bookone")
    dw2 = lib.distinct_words("booktwo")
    if sort_output:
        dw1.sort()
        dw2.sort()
    if sorted(dw1) == sorted(unique_words[0]) and sorted(dw2) == sorted(unique_words[1]):
        print("DISTINCT WORDS CORRECT!")
    else:
        print("DISTINCT WORDS FAILED!")
    
    if lib.count_distinct_words("bookone") == len(unique_words[0]) and lib.count_distinct_words("booktwo") == len(unique_words[1]):
...
def main():
    ...
    texts = [["The", "name", "of", "this", "book", "contains", "a", "number"],["You", "can", "name", "this", "book", "anything"]]
